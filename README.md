...
| 모드 | 입력 | 산출 |
| --- | --- | --- |
| 📝 언어 분석 | 전사 불러오기 · 텍스트 · 음성 | MLU-w, MLU-m, TTR, NDW, TNW, 문법형태소·연결어미 분포 |
| 🔊 조음 분석 | 직접 입력(목표어/산출형) · 음성 | 컨퓨전 매트릭스, PCC(자음)·PVC(모음), 음소별 정확도, 위치별 오류, 오류 음운변동 |
...
├── app.py                       # 메인 (홈 + 모드 카드)
├── pages/
│   ├── 0_🎤_전사.py             # 전사 (음성 → 자동 전사 → 화자 지정 → 분석에 사용/저장)
│   ├── 1_📝_언어_분석.py        # 언어 분석 (전사 불러오기 / 텍스트 / 음성)
│   └── 2_🔊_조음_분석.py        # 조음 분석 (직접 입력/음성 → PCC·PVC·음운변동)
├── modules/
│   ├── __init__.py
│   ├── morpheme.py              # kiwipiepy → MLU/TTR/품사/문장유형
│   ├── transcription.py         # Whisper 목표어 + GPT-4o audio 산출형 전사
│   ├── g2p.py                   # 순수 파이썬 규칙 g2p 래퍼 (목표어 → 발음형)
│   ├── g2p_rules.py             # 한국어 음운변동 규칙 엔진(연음·경음화·비음화 등)
│   ├── jamo_split.py            # 초성/중성/종성 분리
│   ├── phonology.py             # 자음 자질표 + 오류 음운변동(상대분석) 분류
│   ├── articulation.py          # 컨퓨전 매트릭스 + PCC(자음)/PVC(모음) + 음운변동
│   ├── report.py                # 분석 결과 → 인쇄용 HTML 보고서
...
- **✂️ 발화 나누기**: 한 칸에 여러 문장이 묶여 전사된 경우, 셀에 마침표(`.`)나 줄바꿈을 넣고
  ...
  화자·시간 정보는 유지되며, 조음 모드에서는 목표어·산출형이 함께 분리됩니다.
- **🤖 GPT-4o 산출형 자동 생성**(조음 음성 모드): 화자=아동이며 산출형이 빈 행만 채웁니다.
  실패 시 실제 오류 메시지가 표 위에 표시됩니다(API 키·모델 권한·네트워크 확인).

## 직접 입력 모드 (조음 · 권장)

음성 ASR가 한국어 조음장애 아동 말소리에 약해, 조음 분석은 **음성 없이 목표어/산출형을
직접 입력**하는 모드를 기본 제공합니다.
- **목표어**는 표준 철자(예: `먹어요`)로 적거나 전사/엑셀에서 불러옵니다.
- **📝 산출형 = 목표 발음형으로 채우기**: 버튼을 누르면 목표어의 표준 발음형(연음·경음화 등 자연
  변동 적용)이 산출형에 자동으로 채워집니다. **아동이 다르게 낸 음소만 수정**하면 됩니다
  (산출형을 전부 타이핑할 필요 없음).
