...
from modules.insights import summarize_language  # noqa: E402
...
if result:
    ...
    if not age_months:
        st.caption("사이드바에 생년월일·검사일을 입력하면 보고서에 생활연령 기준 언어 발달 규준 비교가 포함됩니다.")

    st.divider()
    with st.expander("📋 분석 요약 (텍스트)"):
        st.text(summarize_language(result))
    with st.expander("📚 연령 기반 언어 발달 규준 (해석 근거)"):
