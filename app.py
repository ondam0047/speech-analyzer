...
with st.expander("ℹ️ 사용 순서 안내"):
    st.markdown(
        """
        0. **사이드바**에 대상자 **생년월일·검사일**을 입력하면 생활연령이 계산되어,
           보고서에 **연령 기반 발달 규준 비교**가 포함됩니다.
        1. **🎤 전사** — 음성을 올려 자동 전사 → 화자(아동/치료사/제외) 지정 → 검수
           (마침표/엔터로 발화 나누기, 끊긴 발화는 합치기) → **분석에 사용**(세션) 또는 **엑셀 저장**.
        2. **📝 언어 분석** — 전사 결과 불러오기 / 텍스트 입력 / 음성 업로드 후 분석.
           MLU·TTR·NDW, 의미영역, 조사·어미 빈도, 문장유형 + 연령 기반 발달 규준 비교.
        3. **🔊 조음 분석** — 전사에서 목표어 불러오기(또는 직접 입력) → **산출형을 목표 발음형으로 채운 뒤**
           아동이 다르게 낸 음소만 수정 → PCC·PVC, 오류 음운변동, 위치별 오류,
           **🗣️ 말명료도**(목표어를 알아들었는지) + 연령 기반 발달 규준 비교.
